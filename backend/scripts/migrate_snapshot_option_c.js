...
const run = async () => {
  ...
  try {
    ...
    if (!hasPlatNom) {
      console.log('Adding column LIGNE_COMMANDE.plat_nom ...');
      await conn.query('ALTER TABLE LIGNE_COMMANDE ADD COLUMN plat_nom VARCHAR(100) NULL');
    }

    if (!hasPlatImageUrl) {
      console.log('Adding column LIGNE_COMMANDE.plat_image_url ...');
      await conn.query('ALTER TABLE LIGNE_COMMANDE ADD COLUMN plat_image_url VARCHAR(255) NULL');
    }

    console.log('Backfilling snapshots from PLAT ...');
    await conn.query(
      `
    ...
    console.log('Ensuring plat_nom is NOT NULL ...');
    await conn.query('ALTER TABLE LIGNE_COMMANDE MODIFY plat_nom VARCHAR(100) NOT NULL');

    console.log('Ensuring id_plat is NULLABLE ...');
    await conn.query('ALTER TABLE LIGNE_COMMANDE MODIFY id_plat INT NULL');
    ...
    if (deleteRule !== 'SET NULL') {
      console.log(`Updating FK ${fkName} to ON DELETE SET NULL ...`);
      ...
      console.log('FK already ON DELETE SET NULL.');
    }

    await conn.commit();
    console.log('Migration Option C (snapshot) completed successfully.');
  } catch (err) {
    try {
      await conn.rollback();
    } catch {
